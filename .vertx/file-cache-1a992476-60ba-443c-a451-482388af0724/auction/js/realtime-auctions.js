...
function loadCurrentPrice() {
    ...
    xmlhttp.open("GET", "http://localhost:8088/api/auctions/" + auction_id);
    xmlhttp.send();
};

function registerHandlerForUpdateCurrentPriceAndFeed() {
    var eventBus = new EventBus('http://localhost:8088/eventbus');
...
function bid() {
    ...
    xmlhttp.open("PATCH", "http://localhost:8088/api/auctions/" + auction_id);
